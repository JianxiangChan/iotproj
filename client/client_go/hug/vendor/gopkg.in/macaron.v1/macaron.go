...
//Run the http server on SetAddr
func (m *Macaron) RunAddr(addr string) {
	host, port := GetDefaultListenInfo()
	if len(addr) == 0 {
		addr = host + ":" + com.ToStr(port)
	}
...
//Run the http server on SetAddr
func (m *Macaron) RunAddrTLS(addr string, cert string, key string) {
	host, port := GetDefaultListenInfo()
	if len(addr) == 0 {
		addr = host + ":" + com.ToStr(port)
	}
	logger := m.GetVal(reflect.TypeOf(m.logger)).Interface().(*log.Logger)
	logger.Printf("listening on https://%s (%s)\n", addr, safeEnv())
